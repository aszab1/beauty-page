...
export default function Products(){
  ...
  const products = [
    ...
    {name: 'MICRONEEDLING', product: 'With INFINI Premium Meso Whitening Max', price: '£175', description: 'This is an innovative formula designed for skin depigmentation and whitening. Enables healthy glow restoration but also leaves it in perfect tone and condition.<br/>In order to obtain the desired effects, perform 4-8 sessions every 10-14 days.'},
    {name: 'MICRONEEDLING', product: 'With Glutanex Glow', price: '£175', description: 'GLUTANEX GLOW is a professional solution for instant brightening, instant tone brightening, pore tightening, and skin regeneration. The combination of Glutathione and PDRN brings up the healthy glow from the inner layers of skin.<br/>**Packages available for a course of treatments, please discuss with your therapist.**'},
  // const masks = [
  ...
  // const peel = [
    {name: 'CHEMICAL PEEL', product: 'INFINI Premium BioAgePeel', price: '£45', description:'Innovative, medical chemical stimulator based on trichloroacetic, kojic and mandelic acid. Recommended 4 to 6 sessions 2 weeks apart'},
...
return(
  ...
  <h2>Price List</h2>
        ...
        {products.map((item, index) => (
          <div key={index}>
            <strong>{item.name}</strong>: {item.product} - {item.price}
          </div>
        ))}
  </div>
      )
